...
public class IPInputView extends EditText {
	...
	private void initProperties(AttributeSet attrs) {
        TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.IPET,0,0);
        final int N = typedArray.getIndexCount();
        for (int i = 0; i < N; ++i) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.IPET_bordColor) {
                bordColor = typedArray.getColor(R.styleable.IPET_bordColor, bordColor);
            } else if (attr == R.styleable.IPET_divideLineWidth) {
                divideLineWidth = typedArray.getDimensionPixelSize(R.styleable.IPET_divideLineWidth, divideLineWidth);
            } else if (attr == R.styleable.IPET_divideLineColor) {
                divideLineColor = typedArray.getColor(R.styleable.IPET_divideLineColor, divideLineColor);
            } else if (attr == R.styleable.IPET_rectAngle) {
                rectAngle = typedArray.getDimensionPixelOffset(R.styleable.IPET_rectAngle, rectAngle);
            } else if (attr == R.styleable.IPET_focusedColor) {
				focusedColor = typedArray.getColor(R.styleable.IPET_focusedColor, focusedColor);
			}
        }
		typedArray.recycle();
    }
